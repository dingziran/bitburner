...
export async function main(ns) {
  ...
  while (true) {
    ...
    const runningHWGWServers = serverObjs
      .flatMap((obj) => obj.processes)
      .filter((pro) => pro.filename === "abc.js" || pro.filename === "test.js")
      .map((pro) => pro.target);
    const hosts = getHosts(ns).filter((host) =>
      targetServer ? host === ns.args[0] : !runningHWGWServers.includes(host)
    );
    const hostObjs = hosts
      .map((host) => {
        const res = {};
        res.hostname = host;
        ...
        res.growGap =
          (ns.getServerMaxMoney(host) || 1) /
          (ns.getServerMoneyAvailable(host) || 1);
        res.runningWeak =
          serverObjs
            .flatMap((obj) => obj.processes)
            .filter((pro) => pro.target === host && pro.filename === weakScript)
            .length > 0;
        res.runningGrow =
          serverObjs
            .flatMap((obj) => obj.processes)
            .filter((pro) => pro.target === host && pro.filename === growScript)
            .length > 0;
        res.runningHack =
          serverObjs
            .flatMap((obj) => obj.processes)
            .filter((pro) => pro.target === host && pro.filename === hackScript)
            .length > 0;
        return res;
      })
...
function weaken(ns, hostObjs, weakEffect, weakRam) {
  ...
  for (const hostObj of sortedHostObjs) {
    ...
    ns.print(
      `${serverToRun.hostname}:weak target:${hostObj.hostname} time:${(
        hostObj.weakTime /
        1000 /
        60
      ).toFixed(1)}m threads:${threadToRun} gap:${hostObj.securityGap.toFixed(
        1
      )} freeRam:${serverToRun.freeRam.toFixed(0)}`
    );
  }
}
...
function grow(ns, hostObjs, growRam) {
  ...
  for (const hostObj of sortedHostObjs) {
    const serverObjs = getServers(ns);
    const serverToRun = getAvailableServer(serverObjs);
    const threadNeeded = Math.ceil(
      ns.growthAnalyze(hostObj.hostname, hostObj.growGap)
    ); // TODO: add core
    ...
    ns.print(
      `${serverToRun.hostname}:grow target:${hostObj.hostname} time:${(
        hostObj.growTime /
        1000 /
        60
      ).toFixed(1)}m threads:${threadToRun} gap:${hostObj.growGap.toFixed(
        1
      )} freeRam:${serverToRun.freeRam.toFixed(0)}`
    );
  }
...
function hack(ns, hostObjs, weakEffect, hackRam, weakRam, growRam) {
  const sortedHostObjs = hostObjs
    .filter((hostObj) => hostObj.securityGap <= 0 && hostObj.growGap <= 1)
    .sort((a, b) => b.money * b.hackChance - a.money * a.hackChance);

  for (const hostObj of sortedHostObjs) {
    const serverObjs = getServers(ns);
    const serverToRun = getAvailableServer(serverObjs);
    const hackThreads = Math.floor(
      ns.hackAnalyzeThreads(hostObj.hostname, hostObj.money * hackThreshold)
    );
    const weakThreadsForHack = Math.ceil(
      ns.hackAnalyzeSecurity(hackThreads) / weakEffect
    );
    const growThreads = Math.ceil(
      ns.growthAnalyze(hostObj.hostname, 1 / hackThreshold)
    );
    const weakThreadsForGrow = Math.ceil(
      ns.growthAnalyzeSecurity(growThreads) / weakEffect
    );
    const threadToRun =
      hackThreads + weakThreadsForGrow + growThreads + weakThreadsForGrow;
    ...
    if (threadToRun > maxThreadAllowed) {
      throw new Error(
        `no available memory ${ramNeeded * threadToRun}/${serverToRun.freeRam}`
      );
    }
    // We need to make scripts to end by hack weak grow weak
    const durationPerBatch = Math.max(
      hostObj.hackTime + scriptInterval * 3,
      hostObj.weakTime + scriptInterval * 2,
      hostObj.growTime + scriptInterval
    );

    ns.exec(
      hackScript,
      serverToRun.hostname,
      hackThreads,
      hostObj.hostname,
      durationPerBatch - hostObj.hackTime - scriptInterval * 3
    );
    ns.exec(
      weakScript,
      serverToRun.hostname,
      weakThreadsForHack,
      hostObj.hostname,
      durationPerBatch - hostObj.weakTime - scriptInterval * 2
    );
    ns.exec(
      growScript,
      serverToRun.hostname,
      growThreads,
      hostObj.hostname,
      durationPerBatch - hostObj.growTime - scriptInterval * 1
    );
    ns.exec(
      weakScript,
      serverToRun.hostname,
      weakThreadsForGrow,
      hostObj.hostname,
      durationPerBatch - hostObj.weakTime
    );
...
function getServers(ns) {
  ...
  const serverObjs = servers.map((hostname) => {
    ...
    for (const process of processes) {
      if (
        [weakScript, growScript, hackScript, "weakH.js", "weakG.js"].includes(
          process.filename
        )
      ) {
        res.processes.push({
...
function getHosts(ns) {
  ...
  for (let i = 0; i < results.length; i++) {
    for (const newHost of ns.scan(results[i])) {
      if (
        results.indexOf(newHost) === -1 &&
        newHost.indexOf("-server-") === -1
      ) {
        if (ns.getServerNumPortsRequired(newHost) <= portsRequired) {
          if (
            ns.getServerRequiredHackingLevel(newHost) <= ns.getHackingLevel()
          ) {
            results.push(newHost);
            if (!ns.hasRootAccess(newHost)) {
              if (ns.fileExists("BruteSSH.exe")) {
                ns.brutessh(newHost);
              }
              if (ns.fileExists("FTPCrack.exe")) {
              ...
              if (ns.fileExists("relaySMTP.exe")) {
                ns.relaysmtp(newHost);
              }
              if (ns.fileExists("HTTPWorm.exe")) {
                ns.httpworm(newHost);
              }
              if (ns.fileExists("SQLInject.exe")) {
                ns.sqlinject(newHost);
              }
              ns.nuke(newHost);
            }
          }
