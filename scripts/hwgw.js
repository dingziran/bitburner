...
export async function main(ns) {
	...
	while (true) {
		...
		if (currentIndex % Math.round(10000 / interval) === 0) {
			...
			ns.print('freeRam ', servers.reduce((m, c) => (m + c.freeRam), 0));
		}
		...
		const weakPosition = Math.floor((Date.now() + weakTime - actionStartTime) / interval * 2) / 2;
		// hack needs to be on 0, 4, 8 ...
		const hackPosition = Math.floor((Date.now() + hackTime - actionStartTime) / interval * 2) / 2;
		// grow needs to be on 2, 6, 10 ...
		const growPosition = Math.floor((Date.now() + growTime - actionStartTime) / interval * 2) / 2;
		...
		if (weakPosition >= 0) {
			if (weakPosition > nextWeakPosition) {
				...
				if (nextWeakPosition % 2 === 1 && jobs[nextWeakPosition] !== weakScript) {
		...
		if (growPosition >= 0) {
			if (growPosition > nextGrowPosition) {
				if (nextGrowPosition % 4 === 2 && jobs[nextGrowPosition] !== growScript) {
					for (let i = 0; i < 4; i++) {
		...
		const growThreads = Math.ceil(ns.growthAnalyze(target, maxMoney / Math.min(curMoney, maxMoney - money), cores)) * (initLevel > 1500 ? 2 : 1);
		const weakForGrowThreads = Math.ceil(ns.growthAnalyzeSecurity(growThreads) / weakEffect);

		if (curMoney < (maxMoney - money) / 2) {
			ns.tprint(`${target} curMoney ${curMoney.toFixed(0)}/${maxMoney.toFixed(0)} ran ${((Date.now() - startTime) / 1000 / 60).toFixed(2)}m lvlup ${ns.getPlayer().hacking - initLevel}`)
		...
		await ns.sleep(Math.max(0, interval / 2 - (Date.now() - currentIndex * interval - startTime)));
